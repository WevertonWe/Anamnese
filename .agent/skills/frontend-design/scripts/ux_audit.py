...
class UXAuditor:
    ...
    def audit_file(self, filepath: str) -> None:
        ...
        for font in font_faces: font_families.add(font.strip().lower())  # noqa: E701
...
def main():
    if len(sys.argv) < 2: sys.exit(1)  # noqa: E701
    
    path = sys.argv[1]
    is_json = "--json" in sys.argv
    
    auditor = UXAuditor()
    if os.path.isfile(path): auditor.audit_file(path)  # noqa: E701
    else: auditor.audit_directory(path)  # noqa: E701
    ...
    if is_json:
        ...
        if report['issues']:
            ...
            for i in report['issues'][:10]: print(f"  - {i}")  # noqa: E701
        if report['warnings']:
            print(f"[*] WARNINGS ({len(report['warnings'])}):")
            for w in report['warnings'][:15]: print(f"  - {w}")  # noqa: E701
