...
export default function AnamneseRemotePage() {
    ...
    try {
        ...
        console.error("Erro ao parsear schema", e);
    }
    const translations = record.template?.translations ? JSON.parse(record.template.translations) : {};

    return (
        <div className="min-h-screen bg-slate-50 p-4 sm:p-8 font-[family-name:var(--font-geist-sans)] pb-32">
            <main className="w-full max-w-2xl mx-auto flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                            ...
                            <div key={field.id} className="flex flex-col">
                                ...
                                {field.type === 'textarea' ? (
                                    <textarea
                                        ...
                                        className="w-full border border-slate-300 rounded-xl p-3 text-slate-900 focus:ring-2 focus:ring-emerald-500 outline-none h-24 resize-none"
                                    ...
                                    <input
                                        ...
                                        value={formData[field.id] || ''}
                                        onChange={e => handleChange(field.id, e.target.value)}
                                        className="w-full border border-slate-300 rounded-xl p-3 text-slate-900 focus:ring-2 focus:ring-emerald-500 outline-none"
                                        placeholder="Digite aqui..."
                                    />
                                )}
                            </div>
                ...
                <div className="fixed bottom-0 left-0 w-full bg-white/90 backdrop-blur-md p-4 border-t border-slate-200 shadow-[0_-10px_40px_rgba(0,0,0,0.05)] z-10 flex justify-center">
                    <button
