...
export default function InsightsPreviewModal({
    ...
    const calcIMC = (): { value: number; label: string; color: string } | null => {
        const pesoRaw = formData['peso'] || formData['peso_kg'] || formData['weight'] || '';
        const alturaRaw = formData['altura'] || formData['altura_cm'] || formData['height'] || '';
        const peso = parseFloat(pesoRaw.replace(',', '.'));
        const alturaVal = parseFloat(alturaRaw.replace(',', '.'));
        if (!peso || !alturaVal || peso <= 0 || alturaVal <= 0) return null;
        
        // Normalização: altura > 3 assume cm, senão metros
        const h = alturaVal > 3 ? alturaVal / 100 : alturaVal;
        const imc = peso / (h * h);
        
        if (imc < 18.5) return { value: imc, label: 'Abaixo do peso', color: 'text-blue-700 bg-blue-50 border-blue-200' };
        if (imc < 25) return { value: imc, label: 'Normal', color: 'text-emerald-700 bg-emerald-50 border-emerald-200' };
        if (imc < 30) return { value: imc, label: 'Sobrepeso', color: 'text-orange-700 bg-orange-50 border-orange-200' };
        if (imc < 35) return { value: imc, label: 'Obesidade I', color: 'text-red-600 bg-red-50 border-red-200' };
        if (imc < 40) return { value: imc, label: 'Obesidade II', color: 'text-red-700 bg-red-50 border-red-200' };
        return { value: imc, label: 'Obesidade III', color: 'text-red-800 bg-red-100 border-red-300' };
    };
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[92vh] flex flex-col animate-in zoom-in-95 duration-200 overflow-hidden">
                ...
                <div className="flex-1 overflow-y-auto p-5">
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        ...
                        {/* Lado Esquerdo: Dados Preenchidos (Split View) */}
                        <div className="flex flex-col h-full border-r border-slate-100 pr-5">
                            ...
                            {isLoading ? (
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div>
                                        ...
                                        <div className="flex flex-wrap gap-1.5 mb-2">
                                            {cidList.length > 0 ? cidList.map((cid, i) => (
                                                <span key={i} className="px-2.5 py-1 bg-blue-50 border border-blue-200 text-blue-700 text-xs font-bold rounded-md flex items-center gap-1">
                                                    {cid}
                                                    <button onClick={() => removeCid(cid)} className="text-blue-400 hover:text-red-500 transition ml-0.5">
                                        ...
                                        <div className="flex gap-1.5">
                                            <input
                                                ...
                                                className="flex-1 border border-blue-200 rounded-lg px-3 py-1.5 text-xs text-slate-800 focus:ring-2 focus:ring-blue-400 outline-none bg-blue-50/40"
                                                placeholder="Ex: G44.2"
                                            />
                                            <button onClick={addCid} className="px-3 py-1.5 text-xs font-bold text-white bg-blue-600 hover:bg-blue-700 rounded-lg transition">+</button>
                                        </div>
                                    </div>
                                    ...
                                    {(() => {
                                        ...
                                        if (!imc) return null;
                                        return (
                                            <div className={`flex items-center gap-3 p-3 rounded-lg border ${imc.color}`}>
                                                <div className="font-black text-2xl">{imc.value.toFixed(1)}</div>
                                                <div>
                                                    <div className="text-[10px] font-bold uppercase tracking-wider opacity-70">IMC</div>
                                                    <div className="text-sm font-bold">{imc.label}</div>
                                                </div>
                                            </div>
                                        );
                                    })()}
