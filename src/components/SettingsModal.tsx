...
export default function SettingsModal({ isOpen, onClose }: { isOpen: boolean, onClose: () => void }) {
    ...
    const handleFileToBase64 = (e: React.ChangeEvent<HTMLInputElement>, setter: (value: string) => void) => {
            ...
            <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col md:flex-row max-h-[90vh]">
                        ...
                        <div className="space-y-4">
                            ...
                            <div>
                                ...
                                <div className="flex flex-col gap-2">
                                    {signatureImage && <img src={signatureImage} alt="Assinatura" className="h-16 object-contain border border-slate-200 rounded-lg p-1 bg-white self-start" />}
                                    <input type="file" accept="image/*" onChange={e => handleFileToBase64(e, setSignatureImage)} className="text-sm text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:bg-emerald-50 file:text-emerald-700 hover:file:bg-emerald-100 cursor-pointer w-full" />
                                </div>
                            </div>
                ...
                <div className="hidden md:flex flex-col w-[300px] bg-slate-100 border-l border-slate-200 p-6 flex-shrink-0">
                    ...
                    <div className="flex-1 bg-white shadow-sm border border-slate-200 aspect-[1/1.4] w-full p-4 flex flex-col justify-between text-[10px] leading-tight text-slate-800 relative">
                        ...
                        <div className={`mt-auto flex flex-col ${signatureAlign === 'center' ? 'items-center text-center' : signatureAlign === 'right' ? 'items-end text-right' : 'items-start text-left'}`}>
                            {signatureImage && (
                                <img src={signatureImage} alt="Assinatura" className="h-8 object-contain mb-1" />
                            )}
                            <div className="w-24 border-t border-slate-800 mb-1"></div>
